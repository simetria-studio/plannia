<aside class="fixed inset-y-0 left-0 z-30 flex w-60 flex-col bg-plannia-navy">
    <div class="flex h-16 items-center px-5 border-b border-white/10">
        <a href="{{ route('dashboard') }}" class="text-white">
            <x-plannia-logo class="text-white" />
        </a>
    </div>

    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        <a href="{{ route('dashboard') }}" @class(['plannia-nav-item', 'plannia-nav-item-active' => request()->routeIs('dashboard')])>
        ...
        <a href="{{ route('students.index') }}" @class(['plannia-nav-item', 'plannia-nav-item-active' => request()->routeIs('documents.*')])>
        ...
        <a href="{{ route('history.index') }}" @class(['plannia-nav-item', 'plannia-nav-item-active' => request()->routeIs('history.*')])>
        ...
        <a href="{{ route('students.index') }}" @class(['plannia-nav-item', 'plannia-nav-item-active' => request()->routeIs('students.*')])>
            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            Alunos e Turmas
        </a>

        <a href="{{ route('history.index', ['tab' => 'pending']) }}" @class(['plannia-nav-item', 'plannia-nav-item-active' => request()->get('tab') === 'pending'])>
        ...
        <a href="{{ route('history.index') }}" class="plannia-nav-item">
        ...
        @if(Auth::user()->isDirecao())
            <a href="{{ route('history.index', ['tab' => 'approved']) }}" class="plannia-nav-item">
                <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
                Compartilhamentos
            </a>

            <a href="{{ route('schools.edit') }}" @class(['plannia-nav-item', 'plannia-nav-item-active' => request()->routeIs('schools.*') || request()->routeIs('turmas.*') || request()->routeIs('users.*')])>
    ...
    <div class="border-t border-white/10 p-4 space-y-3">
        <a href="#" class="flex items-center gap-2 text-sm text-white/50 hover:text-white/80 transition">
        ...
        <x-plannia-logo class="text-white/30 h-5" />
    </div>
</aside>
